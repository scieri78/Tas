...
*/
//CONTROLLER $WFSAdmin=$this->_model->getWFSAdmin($IdWorkFlow, $Uk, $WFSAdmin);

?>
<script>
  function OpenMeterBar<?php echo $IdFlu; ?>(vIdFlu,vIdLegame,vInizio,vDipDiff,vOldDiff,vDipEsito){
    $('#DivMeterBar'+vIdLegame).load('./index.php?controller=workflow2&action=MeterBar.php',{
            IdFLu:    vIdFlu,
            IdLegame: vIdLegame,
            Inizio:   vInizio,
            SecLast:  vDipDiff,
...
*/
// CONTROLLER $ArrElaTest=$this->_model->getArrElaTest($IdWorkFlow, $IdFlu);
...
*/
// CONTROLLER $ArrCarTest=$this->_model->getArrCarTest($IdWorkFlow, $IdFlu);
...
*/
// CONTROLLER $ArrShellDett=$this->_model->getArrShellDett($IdWorkFlow, $IdFlu);
...
//==========   trovo Legami del flusso
    ...
    */
    //CONTROLLER $this->_model->setSvalidaOff($IdWorkFlow, $IdFlu, $IdProcess, $SvalidaOff, $BlockCons);
...
/*
    ...
    */
    // CONTROLLER getArrStato($IdWorkFlow, $IdFlu, $ListIdStato);
   }
...
( SELECT TO_CHAR(MAX(INIZIO),'YYYY-MM-DD HH24:MI:SS') INIZIO FROM WFS.ULTIMO_STATO C WHERE C.ID_WORKFLOW = B.ID_WORKFLOW AND C.TIPO = 'V' AND C.ID_FLU = B.ID_FLU AND C.ID_DIP = B.ID_DIP AND C.INIZIO < B.INIZIO )
  ...
  /* $stmt=db2_prepare($conn, $SqlList);
   ...
   <div style="text-align:center;" ><?php 
    ...
    ?><img style="width: 25px; margin: 10px;" src="./images/Flusso.png" ><B><?php echo "$NomeFlusso";  if ( "$DescFlusso" != "" ){ echo " - $DescFlusso"; }; echo "Permesso:$Permesso - PeriodCons:$PeriodCons - BlockCons:$BlockCons"; ?></B>
    </div>
   </td></tr>
   <?php
      ...
      $DatiLegamiFlussiF = $_model->getLegamiFlussiF($IdWorkFlow, $IdProcess,$ProcMeseEsame,$IdDip);
       /* $SqlList="
        ...
        if ( $CntTTF != 0 ){
           ...
           $Note="";
           $Stato='N';
          $callRes = $_model->callTestSottoFlussi($IdWorkFlow, $IdProcess, $IdFlusso, $Stato, $Errore, $Note);
        ...
        foreach($ArrLegami as $LegamePrio ){
          ...
          if ( $TestPrio < $Prio and $LegamePrio[38] == "N" ){
            ...
            if ( "$TestDipTipo" != "F" ){
               ...
               $Note="";
               $Stato='N';
               $callRes = $_model->callFlussoCompletato($IdWorkFlow,$IdProcess,$TestIdDip,$Stato,$Errore,$Note);
                   ...
                   switch ( $Tipo ) {
                     case "C":
                            ...
                            <tr>
                             <th></th>
                             <td>
                                <?php if ( "$DipLog" != "" ) { ?><img class="ImgIco"  style="height:35px;cursor:pointer;"  src="./images/Log.png"    onclick="OpenLogFileElab('<?php echo $DipLog; ?>','apriLogElab')" ><?php } ?>
                             </td>
                            </tr>
                            <?php
                            break;
                     ...
                     case "V":
                        if ( "$External" == "Y"  ){
                            ...
                            <tr>
                             <th>Log</th>
                             <td>
                                <img class="ImgIco"  style="height:35px;cursor:pointer;"  src="./images/Log.png"    onclick="OpenLogFileElab('/area_staging_TAS/DIR_LOG/RETI_TWS/<?php echo $DipName.'.sh_'.$EserMese.'*'; ?>.log','apriLogElab')" >
                             </td>
                            </tr>
                            <?php } ?>
            ...
            if( "$OldInizio" != ""  and "$OldInizio" != "$OldFine"  ){
                ...
                <table class="ExcelTable OldRun" >
                    ...
                    <tr>
                     <th></th>
                     <td>
                        <?php if ( "$OldLog" != "" ) { ?><img class="ImgIco"  style="height:35px;cursor:pointer;"  src="./images/Log.png"    onclick="OpenLogFileElab('<?php echo $OldLog; ?>','apriLogElab')" ><?php } ?>
...
<script>
  ...
  function OpenProcessing(vIdRunSh){
      window.open('./index.php?sito='+getParameterByName('sito')+'&controller=statoshell&action=index&IDWFM='+vIdRunSh);
  }
  ...
  function OpenLink(vIdLegame,vTarget,vNameDip,vBloccato,vEsitoDip){
     // $('#Waiting').show();
      $('#OpenLinkPage').empty().load('./index.php?controller=workflow&action=OpenLinkPage',{
