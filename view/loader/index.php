...
if ( 1 )  {
   ...
   <aside aria-label="Uploaded icons have their own unique 'fak' prefix" class="fileter-aside starlight-aside starlight-aside--note"> <p class="starlight-aside__title" aria-hidden="true"> <i class="fa fa-regular fa-star-shooting fa-rotate-180"></i>Loader</p> 
			  
				<label class="labelFiltro">MESE ELAB.</label>
     
        <SELECT class="selectSearch" name="SelMeseElab" onchange="$('#FormEserEsame').submit()" style="width:300px;">
        <?php
     ...
     <label class="labelFiltro">PROCESS</label>
    
          <SELECT class="selectSearch" name="Sel_Id_Proc" onchange="$('#FormEserEsame').submit()" style="width:300px;">
          <?php
          ...
          ?></SELECT>
		  <br>
      <label class="labelFiltro">FLUSSO</label>
    
          <SELECT class="selectSearch" name="Sel_Flusso" onchange="$('#FormEserEsame').submit()" style="width:300px;">
          <option value="" >..</option>
          <?php
          $sql = "SELECT DISTINCT FLUSSO FROM WFS.LOAD_ANAG ORDER BY FLUSSO";
          $conn = db2_connect($db2_conn_string, '', '');
          $stmt=db2_prepare($conn, $sql);
          $result=db2_execute($stmt);
          if ( ! $result ){
          ...
          db2_close($db2_conn_string);
          while ($row = db2_fetch_assoc($stmt)) {
            $NOME_FLUSSO=$row['FLUSSO'];
            ?><option value="<?php echo $NOME_FLUSSO; ?>" <?php if ( "$NOME_FLUSSO" == "$Sel_Flusso" ){ ?>selected<?php } ?> ><?php echo $NOME_FLUSSO; ?></option><?php
          }
     
        ?>
        </SELECT>
    <label class="labelFiltro">Load File</label>
  
       
       <input type="file" name="Sel_File" />   
<button id="LoadXls" class="btn" style="display: inline-block;"><i class="fa-solid fa-download"> </i> Download</button>	   
