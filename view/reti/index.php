...
?>
 ...
 <div id="NotShow" name="NotShow" STYLE="height: 100%;width: 55px;position: fixed;background: #bac3d4;z-index: 100;left: 0px;top: 119px;" ></div>
 <div id="filterForm">
 <label for="SET_DATA_ELAB" style="padding-left: 27px;"> DATA ELAB: </label> <SELECT class="selectSearch" name="SET_DATA_ELAB" style="width:150px;margin-left:5px;" onchange="$('#FormRetiTWS').submit()" >
 </div>
 <?php
    ...
    ?></SELECT>
 <div id="DivRef">
  <button id="refresh"  onclick="location.reload();" class="btn"><i class="fa-solid fa-refresh"> </i> Refresh</button>
   ...
   <input type="checkbox" id="ShowHidden"  name="ShowHidden"  value="ShowHidden"  <?php if ( "$ShowHidden"  == "ShowHidden" ){ ?>checked<?php } ?> >
    ...
    <input type="hidden" id="DARETI"           name="DARETI"           value="1" />
...
foreach( $RETI as $RETE ) {
    ...
    //  echo "<br>OldTrueTime:".$OldTrueTime;
     ...
     // $OldTrueTime = $_modelReti->callTrueOldTime($RETE, $ListOldId, $ListDone, $OldTrueTime);
    ...
    //  $TrueTime = $_modelReti->callTrueTime($RETE, $LisIdT, $ListDone, $TrueTime);      
  ...
  <?php
   ...
   <?php if ( $NOW_ESERMESE == "$EserMese" ){ ?>
     ...
     <tr>
         <td class="Esito<?php echo $STATUS; ?>" style="width:30px;color:white;" >Now</td>
            ...
            <li class="liPassi S<?php echo $RETE; ?> " <?php echo $HidePasso; ?> >
               <div class="Old" >
                   ...
                   <tr >
                       ...
                       <?php if ( $ID_RUN_SH != "" ){ ?>
                       <img class="ImgIco" style="height:20px;cursor:pointer;" src="./images/LogProc.png" onclick="OpenLinkReti(<?php echo $ID_RUN_SH; ?>,'<?php echo $PASSO; ?>','<?php echo $RETE; ?>','')" >
                       <?php } ?>
                       <?PHP /*
                   ...
                   <tr>
                       <td class="EsitoN"  style="width:30px;color:white;" >Now</td>
                       <td class="BordBot" ></td>
                       <td class="BordBot" ></td>
                       <td class="BordBot" ></td>
                       <td class="BordBot"  style="width: 30px;" >
   ...
   if ( ! empty(${'New'.$RETE}) ){
        foreach( ${'New'.$RETE} as $NewPasso ) {
           ...
           <li class="liPassi S<?php echo $RETE; ?>" <?php echo $HidePasso; ?> >
               ...
               <div class="New DivP DivP<?php echo $ID_RUN_SH.$PASSO; ?>"  >
                   ...
                   <tr>
                       ...
                       <td class="BordBot" style="width: 30px;"  >
                       <?php if ( "$ID_RUN_SH_OLD" != "" ){ ?>
                       <img class="ImgIco" style="height:20px;cursor:pointer;" src="./images/LogProc.png" onclick="OpenLinkReti(<?php echo $ID_RUN_SH_OLD; ?>,'<?php echo $PASSO; ?>','<?php echo $RETE; ?>','')" >
                       <?php } ?>
                       </td>
                   </tr>
                   </table>
                   ...
                   <tr>
                       ...
                       <td style="width: 30px;" >
                       <?php if ( "$ID_RUN_SH" != "" ){ ?>
                       <img class="ImgIco" style="height:20px;cursor:pointer;" src="./images/LogProc.png" onclick="OpenLinkReti(<?php echo $ID_RUN_SH; ?>,'<?php echo $PASSO; ?>','<?php echo $RETE; ?>','<?php echo $OLD_ESER_MESE; ?>')" >
                       <?php } ?>
                       </td>
                   </tr>
                   </table>
           ...
           <?php 
             ...
             <input type="hidden" id="List<?php echo $RETE.$ID_RUN_SH_OLD; ?>" value="<?php echo $ListId; ?>" />
...
function OpenLinkReti(vIdRunSh,vPasso,vRete,vEserMese){
   
    console.log('OpenLinkReti');
    console.log('LISTOPEN','#List'+vRete+vIdRunSh);
  ...
  var formData={
    ...
    console.log('#ShowDett'+vIdRunSh+vPasso);
 ...
 $.ajax({
        ...
        error: function (stato) {
            errorMessage("OpenLinkReti: Qualcosa è andato storto!", stato);
            $('#Waiting').hide();
            }
        });
      ...
      /*  $('#ShowDett'+vIdRunSh+vPasso).empty().load('./index.php?controller=statoshell&action=contentList',{
        ...
        vListStep=vListStep.replace(','+vPasso+'#'+vIdRunSh+'#'+vRete+'#'+vEserMese,'');
        vListStep=vListStep+','+vPasso+'#'+vIdRunSh+'#'+vRete+'#'+vEserMese;
   } else {
        console.log('OpenLinkReti CLOSE');
        vListStep=vListStep.replace(','+vPasso+'#'+vIdRunSh+'#'+vRete+'#'+vEserMese,'');
        console.log(','+vPasso+'#'+vIdRunSh+'#'+vRete+'#'+vEserMese);
...
$('#FormRetiTWS').submit(function(){
    console.log(' RETI FormRetiTWS submit');
  $('#TopScrollShell1').val($('body').scrollTop());  
  $('#LeftScrollShell1').val($('body').scrollLeft());
});
