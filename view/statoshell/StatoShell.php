...
if ( $find == 1 )  {
    ...
    $ShowUnify=$_POST['ShowUnify'];
    $SkipUnify=$_POST['SkipUnify'];
    if ( "$ShowUnify" == "ShowUnify" ){
        $SkipUnify="";
    }
...
//--------------------------------------------------	
    ...
    <th rowspan="8" style="font-size: 12px;" >
      ...
      ?><input type="checkbox" id="ShowLoader" class="ReloadForm" name="ShowLoader" value="ShowLoader" <?php if ( "$ShowLoader" == "ShowLoader" ){ ?>checked<?php } ?> >Loader<BR><?php 
      if ( "$DB2database" == "TASPCUSR" ){ 
        ?><input type="checkbox" id="ShowUnify" class="ReloadForm" name="ShowUnify" value="ShowUnify" <?php if ( "$ShowUnify" == "ShowUnify" ){ ?>checked<?php } ?> >Unify<BR><?php 
        if ( "$ShowUnify" == "" ){
      ...
      if ( ( "$NumPage" == "$SelNumPage" or "$LastShellRun" == "LastShellRun" or "$DA_RETITWS" != "" ) and ( $CntS < $Soglia or "$SelShTarget" != "" )){
        ...
        if ( "$IdSql" != "" ){
            ...
            <li id="List_<?php echo $IdRunSh."_".$IdSql; ?>" class="has-sub">
                <?php if  ( "$SqlType" == "PLSQL" and "$SqlUseRoutine" != "0" ){    ?><label class="Esplodi">o</label><?php }  ?> 
...
function Refresh(){
  console.log('Refresh StatoShell');
  $('#Waiting').show();
 // $('#FormEserEsame').submit();
};
...
function ChangeA<?php echo $InRetePasso; ?>(vIdSel){
  console.log('ChangeA<?php echo $InRetePasso; ?> StatoShell');
    var vFound=false;
    ...
    console.log('#ListOpenId<?php echo $InRetePasso; ?> StatoShell:'+$('#ListOpenId<?php echo $InRetePasso; ?>').val());
}
...
Refresh(); 
    ...
    function OpenNipote(vIdSh,vIdRunSh){
      console.log('OpenNipote StatoShell');
