...
<form method="POST" >
<?php
if ( isset($_POST['INSELECT']) ) { 
 $inSelect=$_POST['INSELECT']; 
 $vTabella=$_POST['TITOLO'];
?>
	<div class="Titolo" >
	  <input type="text" name="TABELLA" value="<?php echo $vTabella; ?>" readonly class="nascondi" />
      <button onclick="goBack()" class="IcoBack" ><img src="./images/Back.png" style="height:100%;" id="ImgUPD<?php echo $Conta; ?>"/></button>
	  <input class="NomeTab" type="text" value="<?php echo $vTabella; ?>" readonly />  
	  <script>
		function goBack() {
			window.history.back();
		} 
  ...
  ?><div class="Titolo" >
		<input type="hidden" name="TABELLA" value="<?php echo $vTabella; ?>" readonly />
	    <?php if ( "$vTabella" <> "" ){  ?>
		<div id="TitTab" ><?php echo $vTabella; ?></div>
		<?php } ?>
	  </div><?php
}
if ( "$sql" <> "" && !strstr($sql,"DELETE") && !strstr($sql,"UPDATE") ) {
	...
	if ( $Num == 0 ){ $StgTitolo=""; }
  	...
  	<table class="ExcelTable" ><?php echo $StgTitolo ;?>
  	<?php
