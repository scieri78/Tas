...
if ( $datishell->ShowSourceSh == "Y" ){ 
  $iconAction.='<img src="./images/File.png" class="IconFile" onclick="openDialog('.$FIdSh.',\'File :'.$FShName.'\',\'apriFile\')">'; 
}
if ($FShLog != "" ){
  $iconAction.='<img src="./images/Log.png" onclick="openDialog('.$FIdRunSh.',\'Log: '.$FShName.'\',\'apriLog\')" class="IconSh">';
}                  
$iconAction.='<img src="./images/Graph.png" onclick="openGrafici(\''.$FShName.'\',\''.$FShTags.'\',\''.$FIdSh.'\')" class="IconSh">';

if ("$FShTabTouch" != "0") {
  $iconAction.='<img src="./images/PlsqlTab.png" onclick="openRelTab(\''.$FIdSh.'\',\''.$IdRunSh.'\',\'\',\''.$FShName.'\')" class="IconSh" >';
}
if ($FShMail == "Y"){
...
if (( 1==1
     ...
     and ( $FSecLast - $FSecPre > $datishell->GAP or $FSecLast - $FSecPre < -$datishell->GAP )
     ) or ( "$FShStatus" == "I" )
    ) {
        ...
        $progresElement .='" role="progressbar" aria-valuenow="'.$FPerc.'" aria-valuemin="0" aria-valuemax="100" style="text-align:right;padding-right: 5px;min-width: 2em; width:'; 
        if ($Perc > 100) {
        $progresElement .= ' 100';
              } else {
               $progresElement .=$FPerc;
              } 
       $progresElement .='%;background-color: '.$FSColor.';height: 100%;float:left;border-radius: 5px;" ><LABEL style="font-weight: 500;">';
              if ($FPerc > 100) {
                  $FPerc = $FPerc - 100;
                  $FPerc = "+" . $FPerc;
              } else {
        ...
        $progresElement .= $FPerc.'%</LABEL></div></div>';
 ...
 ?> 
   ...
   <a onclick="ChangeA<?php echo $FInRetePasso; ?>('<?php echo $FIdRunSh; ?>')" >
               <table class="ExcelTable L1Def Tabella4" style="box-shadow: 0px 0px 0px 1px black;" >
                    <tr>
                        <td rowspan=2 style="min-width:3px" class="<?php echo $FEsito; ?>" ></td>
                        <td rowspan=2 class="openLi" style="min-width:50px" title='<?php echo $FShMessage; ?>' >RC:<?php echo $FShRc; ?></td>
                        <td rowspan=2 class="openLi"  style="cursor:pointer;min-width:320px" ><B><?php echo $FShName; ?></B></td> 
						<td rowspan=2><img title="<?php echo $FIdRunSh; ?>" src="./images/Shell.png" class="IconFile"  onclick="OpenShSel(<?php echo $FIdRunSh; ?>);" /></td>
