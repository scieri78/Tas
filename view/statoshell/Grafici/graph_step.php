...
MESE_ELAB,
timestampdiff(2,
	(SELECT END_TIME   FROM WORK_CORE.CORE_SH WHERE ID_RUN_SH = b.ID_RUN_SH )
     -
	(SELECT START_TIME FROM WORK_CORE.CORE_SH WHERE ID_RUN_SH = b.ID_RUN_SH )
) DURATA
FROM (
	SELECT 
		ID_SH,
		TO_CHAR(START_TIME,'YYYYMM') MESE_ELAB,
	...
	AND STATUS IN ('F','W')
	GROUP BY ID_SH,TO_CHAR(START_TIME,'YYYYMM')
	ORDER BY ID_SH,TO_CHAR(START_TIME,'YYYYMM')
) 
";
