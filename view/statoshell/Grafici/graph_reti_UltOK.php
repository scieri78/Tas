...
while ($row = mysql_fetch_assoc($rt)) {
        ...
        if ( $Durata > $SogliaOra ) {
          ...
          $IsQuarter=0;
          $sIsQuarter="select count(*) Conta from STAT_QUARTER where RETE='$Rete' ";
          $rIsQuarter = mysql_query($sIsQuarter);
          while ($rwIsQuarter = mysql_fetch_assoc($rIsQuarter)) { $IsQuarter=$rwIsQuarter["Conta"]; }    
      
	  
	      /*
          $NLoadList ="";
          if ( $IsQuarter != 0 ) { 
           $NLoadList = " and SUBSTR(NLOAD,5,6) in ('03','06','09','12') ";
          } else {
           $NLoadList = " and SUBSTR(NLOAD,5,6) not in ('03','06','09','12') ";
          }		  
          $VLoad="";
		  $sNL="select max(NLOAD) NLOAD from STAT_LOG_STEP where RETE='$Rete' and NLOAD <= $NLoad-100 $NLoadList and STATO='F'";
          */
		  $sNL="select max(NLOAD) NLOAD from STAT_LOG_STEP where RETE='$Rete' and NLOAD = $NLoad-100 and STATO='F'";
          $rNL = mysql_query($sNL);
          while ($rwNL = mysql_fetch_assoc($rNL)) {
            $VLoad=$rwNL["NLOAD"];
          }
          $x_axis[$i]="$Rete (Prec.$VLoad)";
          if ( $Durata > (  60 * 60 * 30 ) ) { $Durata=( 60 * 60 * 30 )+( 60 * 60 * 2); }
          $y_axis[$i]=$Durata;
          
          $DurataPre=TempoRealeRete($Rete,$VLoad);  
