...
<div class="processing-list-container">
    ...
    <table class="processing-table">
        <tbody>
            <?php
            if (is_array($DatiArrayShell) && count($DatiArrayShell) > 0) {
                foreach ($DatiArrayShell as $row) {
                    $idSh = $row['ID_SH'];
                    ...
                    if (isset($row['OLD_TIME']) && $row['OLD_TIME'] !== '') {
                        ...
                        if ($duration !== '' && $oldSeconds > 0) {
                            $durationSeconds = strtotime('1970-01-01 ' . $duration . ' UTC');
                            if ($durationSeconds !== false) {
                                $meter = (string) round(($durationSeconds / $oldSeconds) * 100) . '%';
                            }
                        }
                    }
