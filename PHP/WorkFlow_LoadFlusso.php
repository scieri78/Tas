...
( SELECT TO_CHAR(MAX(INIZIO),'YYYY-MM-DD HH24:MI:SS') INIZIO FROM WFS.ULTIMO_STATO C WHERE C.ID_WORKFLOW = B.ID_WORKFLOW AND C.TIPO = 'V' AND C.ID_FLU = B.ID_FLU AND C.ID_DIP = B.ID_DIP AND C.INIZIO < B.INIZIO )
        ...
        SELECT L.ID_WORKFLOW,ID_LEGAME,PRIORITA,L.TIPO,L.ID_DIP,US.WARNING,L.ID_FLU,
          ...
          ON 1=1
           ...
           WHERE 1=1
                ...
                AND INZ_VALID <= ( SELECT TO_CHAR(DATA_APERTURA,'YYYYMM') FROM WORK_CORE.ID_PROCESS WHERE ID_PROCESS = $IdProcess )
                AND FIN_VALID >= ( SELECT TO_CHAR(DATA_APERTURA,'YYYYMM') FROM WORK_CORE.ID_PROCESS WHERE ID_PROCESS = $IdProcess )
                AND L.ID_WORKFLOW  = $IdWorkFlow
                AND L.ID_FLU       = $IdFlu
        ) b
   ) a
   ...
   <img id="ImgRefresh" src="../images/refresh.png" width="25" height="25" style="position:fixed;top: 251px;left: 72.5%;background:white;border:1px solid black;" onclick="ReOpenDettFlusso(<?php echo $IdWorkFlow; ?>,<?php echo $IdProcess; ?>,<?php echo $IdFlu; ?>,'<?php echo $NomeFlusso; ?>','<?php echo $DescFlusso; ?>')" >
   ...
   foreach($ArrLegami as $Legame ){
    ...
    if ( "$Tipo"  == "F" ){
        ...
        switch ( $Tipo ) {
         case "C":
           $ImgTipo="Carica";
           $SqlList="SELECT CAMPO_1 TARGTAB FROM WORK_RULES.LOAD_TRASCODIFICA WHERE NOME_FLUSSO = '$DipName' AND TIPO_RECORD = 'T'";
           $stmt=db2_prepare($conn, $SqlList);
           db2_execute($stmt);
           while ($row = db2_fetch_assoc($stmt)) {
               $TarTab=$row['TARGTAB'];
