...
while ($row = mysql_fetch_assoc($rt)) {
        ...
        if ( $Durata > $SogliaOra ) {
          ...
          $y_axis[$i]=$Durata;
          $NLOADPRE="";
          $sNL="select max(NLOAD) NLOAD from STAT_LOG_STEP where NLOAD < $NLoad and RETE = '$Rete' and STATO='F'";
          $rNL = mysql_query($sNL);
          while ($rwNL = mysql_fetch_assoc($rNL)) { $NLOADPRE=$rwNL["NLOAD"]; }
          $x_axis[$i]="$Rete(Prec: $NLOADPRE)";        
...
$p2->SetCenter();
$p2->SetLegend('Mese Precedente Term.Corr.');
