...
<table class="ExcelTable" style="width:800px;margin:auto;left:0;right:0;">
	...
	$sql="SELECT DB_NAME, APPL_NAME, AUTHID, h.AGENT_ID, TBSP_NAME, TABSCHEMA, TABNAME, LOCK_OBJECT_TYPE, LOCK_MODE, LOCK_STATUS, count(*) CNT,
DECODE(SCHEMA,'','',SCHEMA || '.' || PACKAGE || '.' || ROUTINE) PACKAGE, ID_RUN_SH
FROM SYSIBMADM.LOCKS_HELD h
INNER JOIN WORK_ELAB.LOG_ROUTINES l ON l.AGENT_ID = h.AGENT_ID
INNER JOIN WORK_CORE.CORE_DB db ON db.ID_RUN_SQL = l.ID_RUN_SQL
WHERE TABSCHEMA NOT LIKE 'IBM%'
--  AND TABNAME = 'SEG_DIN'
GROUP BY DB_NAME, APPL_NAME, AUTHID, h.AGENT_ID,TBSP_NAME, TABSCHEMA, TABNAME, LOCK_OBJECT_TYPE, LOCK_MODE, LOCK_STATUS, SCHEMA,PACKAGE, ROUTINE, ID_RUN_SH
ORDER BY  TABSCHEMA, TABNAME, LOCK_OBJECT_TYPE, LOCK_MODE, LOCK_STATUS, SCHEMA,PACKAGE, ROUTINE
	";
	...
	while ($row = db2_fetch_assoc($stmt)) {
	  ...
	  <tr>
		...
		<td><?php echo $CNT; ?></td> 
		<th onclick="OpenShSel('<?php echo $ID_RUN_SH_RETE; ?>');"><?php echo $PACKAGE; ?></th>
